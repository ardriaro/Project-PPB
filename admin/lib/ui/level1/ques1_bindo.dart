...
class Ques1Bindo extends StatefulWidget {
  final int level;
  Ques1Bindo({required this.level});

  @override
  _Ques1BindoState createState() => _Ques1BindoState();
}

class _Ques1BindoState extends State<Ques1Bindo> {
  ...
  void _saveText() async {
    ...
    if (_docId == null) {
      DocumentReference docRef = await FirebaseFirestore.instance.collection('questions').add({
      ...
      await FirebaseFirestore.instance.collection('questions').doc(_docId).update({
        'question_text': _textController.text,
        'answers': _fieldControllers.map((controller) => {
          'text': controller.text,
        }).toList(),
  ...
  void _deleteData() async {
    if (_docId != null) {
      await FirebaseFirestore.instance.collection('questions').doc(_docId).delete().then((value) {
