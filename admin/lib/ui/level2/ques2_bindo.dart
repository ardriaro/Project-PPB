...
class Ques2Bindo extends StatefulWidget {
  final int level;
  Ques2Bindo({required this.level});

  @override
  _Ques1BindoState createState() => _Ques1BindoState();
}

class _Ques1BindoState extends State<Ques2Bindo> {
  ...
  void _saveText() async {
    if (_textController.text.isEmpty ||
      ...
      return; // Jangan lakukan apapun jika ada kolom yang kosong
    }

    if (_docId == null) {
      DocumentReference docRef = await FirebaseFirestore.instance.collection('questions2').add({
      ...
      await FirebaseFirestore.instance.collection('questions2').doc(_docId).update({
        'question_text': _textController.text,
        'answers': _fieldControllers.map((controller) => {
          'text': controller.text,
        }).toList(),
  ...
  void _deleteData() async {
    if (_docId != null) {
      await FirebaseFirestore.instance.collection('questions2').doc(_docId).delete().then((value) {
