...
class Level2_mtk extends StatefulWidget {
  @override
  _LevelScreenState createState() => _LevelScreenState();
}

class _LevelScreenState extends State<Level2_mtk> {
  ...
  void navigateToLevelPage(BuildContext context, int level) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => Ques2Mtk(level: level),
      ),
    ).then((shouldUpdate) {
  ...
  void addNewLevel() async {
    final bool? shouldAdd = await Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => Ques2Mtk(level: levels.length + 1)),
    );
